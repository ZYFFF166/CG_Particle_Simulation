...
vec3 Get_Particle_Color(float d)
{
    if(d < 0.1)
        ...
        return Interpolate(vec3(1, 0, 0), vec3(0.5, 0.5, 0.5), (d - 2) / 1);
    else
...
struct Particle
{
	...
	void Handle_Collision(float damping, float coeff_restitution)
	{
		if (this->position[1]<0)
		{
			...
			{	
				this->velocity[1] = -coeff_restitution * velocity[1];
...
void Add_Particles(int n)
{
	for( int i =0;i<n;i++)
	{
		...
		particle.mass = 1.0;
		particle.d = 0.0;
		particle.position = {random(0.0,0.2),0.05,random(0.0,0.2)};
		particle.velocity = {10*particle.position[0],random(1.0,10.0),10*particle.position[2]};
		particle.color = {1.0,1.0,0.0};
...
void application::draw_event()
{
    ...
    if (!paused) 
    {
        ...
        for( int i =0; i<Particles.size();i++)
        {
        	...
        	Particles[i].force = Particles[i].force + vec3(0.0, -Particles[i].mass*g,0.0);
    ...
    for( int i = 0;i<Particles.size();i++)
    {
    	...
    	glVertex3f(Particles[i].position[0],Particles[i].position[1],Particles[i].position[2]);
  		vec3 po = Particles[i].position + static_cast <float>(0.04)*Particles[i].velocity;
  		//po = {po[0],po[1],po[2]}
